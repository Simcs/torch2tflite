...
def onnx2tf(onnx_model, args: Namespace):
    tf_model_path = f'./models/{args.model_name}_tf'
...
def test_conversion(
    ...
    for i in tqdm(range(args.n_test)):
        ...
        tf_outputs = tf_model(**{'input': tf_input})
        inference_speeds['tensorflow'].append(time.time() - start)

        tf_outputs_numpy = tf_outputs['output'].numpy()
...
def main(args: Namespace):
    ...
    # ===== 3. ONNX => Tensorflow =====
    tf_model, tf_path = onnx2tf(onnx_model, args)
    ...
    inference_speeds = test_conversion(torch_model, onnx_path, tf_model, tflite_path, args)
